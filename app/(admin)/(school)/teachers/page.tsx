...
export default function TeachersPage() {
  ...
  const handleAddField = () => {
    const fieldName = window.prompt("Enter the new Excel field name")?.trim()
    ...
    setColumns((currentColumns) => [...currentColumns, fieldName])
    setRows((currentRows) =>
      currentRows.map((row) => ({ ...row, [fieldName]: "" })),
    )
    setDraftRow((currentDraft) => ({ ...currentDraft, [fieldName]: "" }))
    setError("")
    setSuccessMessage("")
  }
