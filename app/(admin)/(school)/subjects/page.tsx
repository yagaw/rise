...
const mockSubjects: Subject[] = [
  {
    ...
    title_english: "Mathematics",
    title_short: "Math",
    title_myanmar: "သင်္ချာ",
  },
  {
    id: "2",
    created_at: "2024-05-02",
    title_english: "Science",
    title_short: "Sci",
    title_myanmar: "သိပ္ပံ",
  },
]

type SortField = keyof Subject
...
export default function SubjectsPage() {
  ...
  const filtered = useMemo(() => {
    let filtered = subjects.filter((s) => {
      ...
      return (
        ...
        (s.title_short || "").toLowerCase().includes(q) ||
        (s.title_myanmar || "").toLowerCase().includes(q)
      )
    })
  ...
  const handleDelete = (id: string) => {
    if (confirm("Delete this subject?")) {
      ...
      // TODO: call API to delete subject
    }
  }
              ...
              <TableRow className="border-b border-gray-100 dark:border-gray-800">
                <TableCell className="px-5 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                  <div
                    ...
                    onClick={() => handleSort("title_english" as SortField)}
                    role="button"
                  >
                    Title (English)
                    {renderSortIcon("title_english" as SortField)}
                  </div>
                </TableCell>
                <TableCell className="px-5 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                  Short
