...
type AvailableIeCoFilter = IeCoFilter & {
  column: string
}
...
export default function IeCoPage() {
  ...
  const [loadingIeCo, setLoadingIeCo] = useState(false)
  ...
  useEffect(() => {
    ...
    const loadExcelFiles = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/ie-co/excel?${params.toString()}`)
        const body = (await response.json()) as LoadResponse

        if (!response.ok) {
          throw new Error(body.error || "Failed to load IE CO Excel files.")
        }
        ...
        const message =
          loadError instanceof Error
            ? loadError.message
            : "Failed to load IE CO Excel files."
        setExcelFiles([])
        setSelectedExcelFileId("")
        setError(message)
      } finally {
  ...
  const availableFilters = useMemo(() => {
    const filters: AvailableIeCoFilter[] = []
  ...
  const handleLoadIeCo = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to load IE CO Excel file.")
      }
      ...
      const message =
        loadError instanceof Error
          ? loadError.message
          : "Failed to load IE CO Excel file."
      setError(message)
    } finally {
      setLoadingIeCo(false)
    }
  }
  ...
  const handleSave = async () => {
    if (!excelFile?.id) {
      setError("Load an IE CO Excel file before saving.")
      return
    }
    ...
    try {
      const response = await fetch("/api/ie-co/excel", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to save IE CO Excel file.")
      }

      setColumns(body.columns ?? columns)
      setRows(body.rows ?? rows)
      setSuccessMessage("IE CO Excel file saved.")
    } catch (saveError) {
      const message =
        saveError instanceof Error
          ? saveError.message
          : "Failed to save IE CO Excel file."
      setError(message)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="min-w-0">
      <SettingsButtons />
      <PageBreadcrumb pageTitle="IE CO" />
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <Button
              size="sm"
              onClick={handleLoadIeCo}
              disabled={!selectedDataYear || !selectedExcelFileId || loadingIeCo}
              isLoading={loadingIeCo}
        ...
        {!excelFile && (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Select a data year and data record to load the IE CO Excel file.
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              IE CO data uses excel_data records with data_type 12 by default.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
                  {excelFile.name || "IE CO Excel File"}
                </h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          ...
          <div className="mx-auto flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-gray-900 lg:max-h-[calc(100dvh-3rem)] lg:max-w-5xl">
            <div className="relative shrink-0 border-b border-gray-100 px-5 py-4 pr-16 dark:border-gray-800 sm:px-6 sm:py-5">
              ...
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                {editingRowIndex === null ? "Add IE CO Row" : "Edit IE CO Row"}
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
