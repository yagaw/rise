...
const mockRows: StateRegion[] = [
  {
    id: "1",
    created_at: "2026-01-19 06:22:39",
    title_english: "Chin",
    title_myanmar: "ချင်းပြည်နယ်",
  },
]

type SortField = keyof StateRegion
...
export default function StateRegionPage() {
  const [rows, setRows] = useState<StateRegion[]>(mockRows)
  ...
  const filtered = useMemo(() => {
    ...
    let filtered = rows.filter(
      (r) =>
        r.title_english.toLowerCase().includes(q) ||
  ...
  const handleDelete = (id: string) => {
    if (confirm("Delete this state/region?")) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="State / Region" />
              ...
              <TableRow className="border-b border-gray-100 dark:border-gray-800">
                <TableCell className="px-5 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                  <div
                    ...
                    onClick={() => handleSort("title_english")}
                  >
                    Title (English){renderSortIcon("title_english")}
                  </div>
                </TableCell>
                <TableCell className="px-5 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                  Title (Myanmar)
                ...
                >
                  ...
                  <TableCell className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <Link href={`/state_region/edit/${r.id}`}>
