...
type IeTeacherFilter = {
  key: string
  label: string
  candidates: string[]
}

type AvailableIeTeacherFilter = IeTeacherFilter & {
  column: string
}
...
export default function IeTeacherPage() {
  ...
  const [loadingIeTeacher, setLoadingIeTeacher] = useState(false)
  ...
  useEffect(() => {
    ...
    const loadExcelFiles = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/ie-teacher/excel?${params.toString()}`)
        const body = (await response.json()) as LoadResponse

        if (!response.ok) {
          throw new Error(body.error || "Failed to load IE Teacher Excel files.")
        }
        ...
        const message =
          loadError instanceof Error
            ? loadError.message
            : "Failed to load IE Teacher Excel files."
        setExcelFiles([])
        setSelectedExcelFileId("")
        setError(message)
      } finally {
  ...
  const availableFilters = useMemo(() => {
    const filters: AvailableIeTeacherFilter[] = []
  ...
  const handleLoadIeTeacher = async () => {
    if (!selectedDataYear) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to load IE Teacher Excel file.")
      }
      ...
      const message =
        loadError instanceof Error
          ? loadError.message
          : "Failed to load IE Teacher Excel file."
      setError(message)
    } finally {
      setLoadingIeTeacher(false)
    }
  }
  ...
  const handleSave = async () => {
    if (!excelFile?.id) {
      setError("Load an IE Teacher Excel file before saving.")
      return
    }
    ...
    try {
      const response = await fetch("/api/ie-teacher/excel", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to save IE Teacher Excel file.")
      }

      setColumns(body.columns ?? columns)
      setRows(body.rows ?? rows)
      setSuccessMessage("IE Teacher Excel file saved.")
    } catch (saveError) {
      const message =
        saveError instanceof Error
          ? saveError.message
          : "Failed to save IE Teacher Excel file."
      setError(message)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="min-w-0">
      <SettingsButtons />
      <PageBreadcrumb pageTitle="IE Teacher" />
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <Button
              size="sm"
              onClick={handleLoadIeTeacher}
              disabled={!selectedDataYear || !selectedExcelFileId || loadingIeTeacher}
              isLoading={loadingIeTeacher}
        ...
        {!excelFile && (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Select a data year and data record to load the IE Teacher Excel file.
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              IE Teacher data uses excel_data records with data_type 9 by default.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
                  {excelFile.name || "IE Teacher Excel File"}
                </h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          ...
          <div className="mx-auto flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-gray-900 lg:max-h-[calc(100dvh-3rem)] lg:max-w-5xl">
            <div className="relative shrink-0 border-b border-gray-100 px-5 py-4 pr-16 dark:border-gray-800 sm:px-6 sm:py-5">
              ...
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                {editingRowIndex === null ? "Add IE Teacher Row" : "Edit IE Teacher Row"}
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
