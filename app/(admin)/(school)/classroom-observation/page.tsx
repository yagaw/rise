...
type ClassroomObservationFilter = {
  key: string
  label: string
  candidates: string[]
}

type AvailableClassroomObservationFilter = ClassroomObservationFilter & {
  column: string
}
...
] satisfies ClassroomObservationFilter[]
...
export default function ClassroomObservationPage() {
  ...
  const [loadingClassroomObservation, setLoadingClassroomObservation] =
    useState(false)
    ...
    const loadExcelFiles = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/classroom-observation/excel?${params.toString()}`,
        )
        const body = (await response.json()) as LoadResponse

        if (!response.ok) {
          throw new Error(
            body.error || "Failed to load Classroom Observation Excel files.",
          )
        }
        ...
        const message =
          loadError instanceof Error
            ? loadError.message
            : "Failed to load Classroom Observation Excel files."
        setExcelFiles([])
        setSelectedExcelFileId("")
        setError(message)
      } finally {
  ...
  const availableFilters = useMemo(() => {
    const filters: AvailableClassroomObservationFilter[] = []
  ...
  const handleLoadClassroomObservation = async () => {
    if (!selectedDataYear) {
    ...
    setLoadingClassroomObservation(true)
    setError("")
    setSuccessMessage("")
    clearWorkbook()
    ...
    try {
      ...
      const response = await fetch(`/api/classroom-observation/excel?${params.toString()}`)
      const body = (await response.json()) as LoadResponse

      if (!response.ok) {
        throw new Error(body.error || "Failed to load Classroom Observation Excel file.")
      }
      ...
      const message =
        loadError instanceof Error
          ? loadError.message
          : "Failed to load Classroom Observation Excel file."
      setError(message)
    } finally {
      setLoadingClassroomObservation(false)
    }
  }
  ...
  const handleSave = async () => {
    if (!excelFile?.id) {
      setError("Load a Classroom Observation Excel file before saving.")
      return
    }
    ...
    try {
      const response = await fetch("/api/classroom-observation/excel", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to save Classroom Observation Excel file.")
      }

      setColumns(body.columns ?? columns)
      setRows(body.rows ?? rows)
      setSuccessMessage("Classroom Observation Excel file saved.")
    } catch (saveError) {
      const message =
        saveError instanceof Error
          ? saveError.message
          : "Failed to save Classroom Observation Excel file."
      setError(message)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="min-w-0">
      <SettingsButtons />
      <PageBreadcrumb pageTitle="Classroom Observation" />
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <Button
              size="sm"
              onClick={handleLoadClassroomObservation}
              disabled={!selectedDataYear || !selectedExcelFileId || loadingClassroomObservation}
              isLoading={loadingClassroomObservation}
              className="h-11 whitespace-nowrap"
            >
              Load Classroom Observation
            </Button>
          </div>
        </div>
        ...
        {!excelFile && (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Select a data year and data record to load the Classroom Observation Excel
              file.
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Classroom Observation data uses excel_data records with data_type 6
              by default.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
                  {excelFile.name || "Classroom Observation Excel File"}
                </h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {availableFilters.map((filter) => (
                    <div key={filter.key} className="min-w-0">
                      <label
                        htmlFor={`classroom-observation-filter-${filter.key}`}
                        className="mb-1.5 block text-xs font-medium text-gray-500 dark:text-gray-400"
                      >
                        {filter.label}
                      </label>
                      <select
                        id={`classroom-observation-filter-${filter.key}`}
            ...
            <div className="relative shrink-0 border-b border-gray-100 px-5 py-4 pr-16 dark:border-gray-800 sm:px-6 sm:py-5">
              ...
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                {editingRowIndex === null
                  ? "Add Classroom Observation Row"
                  : "Edit Classroom Observation Row"}
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
