...
type AvailableEccdCoFilter = EccdCoFilter & {
  column: string
}
...
export default function EccdCoPage() {
  ...
  const [loadingEccdCo, setLoadingEccdCo] =
    useState(false)
    ...
    const loadExcelFiles = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/eccd-co/excel?${params.toString()}`,
        )
        const body = (await response.json()) as LoadResponse

        if (!response.ok) {
          throw new Error(
            body.error || "Failed to load ECCD CO Excel files.",
          )
        }
        ...
        const message =
          loadError instanceof Error
            ? loadError.message
            : "Failed to load ECCD CO Excel files."
        setExcelFiles([])
        setSelectedExcelFileId("")
        setError(message)
      } finally {
  ...
  const availableFilters = useMemo(() => {
    const filters: AvailableEccdCoFilter[] = []
  ...
  const handleLoadEccdCo = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/eccd-co/excel?${params.toString()}`)
      const body = (await response.json()) as LoadResponse

      if (!response.ok) {
        throw new Error(body.error || "Failed to load ECCD CO Excel file.")
      }
      ...
      const message =
        loadError instanceof Error
          ? loadError.message
          : "Failed to load ECCD CO Excel file."
      setError(message)
    } finally {
      setLoadingEccdCo(false)
    }
  }
  ...
  const handleSave = async () => {
    if (!excelFile?.id) {
      setError("Load an ECCD CO Excel file before saving.")
      return
    }
    ...
    try {
      const response = await fetch("/api/eccd-co/excel", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to save ECCD CO Excel file.")
      }

      setColumns(body.columns ?? columns)
      setRows(body.rows ?? rows)
      setSuccessMessage("ECCD CO Excel file saved.")
    } catch (saveError) {
      const message =
        saveError instanceof Error
          ? saveError.message
          : "Failed to save ECCD CO Excel file."
      setError(message)
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="min-w-0">
      <SettingsButtons />
      <PageBreadcrumb pageTitle="ECCD CO" />
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <Button
              size="sm"
              onClick={handleLoadEccdCo}
              disabled={!selectedDataYear || !selectedExcelFileId || loadingEccdCo}
              isLoading={loadingEccdCo}
        ...
        {!excelFile && (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Select a data year and data record to load the ECCD CO Excel
              file.
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              ECCD CO data uses excel_data records with data_type 8
              by default.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
                  {excelFile.name || "ECCD CO Excel File"}
                </h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
