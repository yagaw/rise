...
type AvailableQleFilter = QleFilter & {
  column: string
}
...
export default function SchoolQlePage() {
  ...
  const [loadingQle, setLoadingQle] = useState(false)
    ...
    const loadExcelFiles = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/school-qle/excel?${params.toString()}`,
        )
        const body = (await response.json()) as LoadResponse

        if (!response.ok) {
          throw new Error(
            body.error || "Failed to load School BE QLE Excel files.",
          )
        }
        ...
        const message =
          loadError instanceof Error
            ? loadError.message
            : "Failed to load School BE QLE Excel files."
        setExcelFiles([])
        setSelectedExcelFileId("")
        setError(message)
      } finally {
  ...
  const availableFilters = useMemo(() => {
    const filters: AvailableQleFilter[] = []
  ...
  const handleLoadQle = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/school-qle/excel?${params.toString()}`)
      const body = (await response.json()) as LoadResponse

      if (!response.ok) {
        throw new Error(body.error || "Failed to load School BE QLE Excel file.")
      }
  ...
  const handleSave = async () => {
    if (!excelFile?.id) {
      setError("Load a School BE QLE Excel file before saving.")
      return
    }
    ...
    try {
      const response = await fetch("/api/school-qle/excel", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(body.error || "Failed to save School BE QLE Excel file.")
      }

      setColumns(body.columns ?? columns)
      setRows(body.rows ?? rows)
      setSuccessMessage("School BE QLE Excel file saved.")
    } catch (saveError) {
      const message =
  ...
  return (
    <div className="min-w-0">
      <SettingsButtons />
      <PageBreadcrumb pageTitle="School BE QLE" />
      ...
      <div className="min-w-0 overflow-hidden rounded-xl border border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="border-b border-gray-100 p-5 dark:border-gray-800">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <Button
              size="sm"
              onClick={handleLoadQle}
              disabled={!selectedDataYear || !selectedExcelFileId || loadingQle}
              isLoading={loadingQle}
        ...
        {!excelFile && (
          <div className="p-10 text-center">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Select a data year and data record to load the School BE QLE Excel
              file.
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              School BE QLE data uses excel_data records with data_type 5.
            </p>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 p-4 dark:border-gray-800 sm:p-5">
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
                  {excelFile.name || "School BE QLE Excel File"}
                </h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <div className="relative shrink-0 border-b border-gray-100 px-5 py-4 pr-16 dark:border-gray-800 sm:px-6 sm:py-5">
              ...
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
                {editingRowIndex === null
                  ? "Add School BE QLE Row"
                  : "Edit School BE QLE Row"}
              </h3>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
