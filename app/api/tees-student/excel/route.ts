...
const TEES_STUDENT_DATA_TYPE_ID = process.env.TEES_STUDENT_DATA_TYPE_ID || "16"
...
async function listStudentExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", TEES_STUDENT_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load Student TEES Excel files: ${error.message}`)
  }
...
async function findStudentExcelRecordById(
  ...
  if (error) {
    throw new Error(`Failed to find Student TEES Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The Student TEES Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Student TEES")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!record) {
      return NextResponse.json(
        { error: "No Student TEES Excel file found for the selected data year." },
        { status: 404 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to load Student TEES Excel file."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to save Student TEES Excel file."
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
