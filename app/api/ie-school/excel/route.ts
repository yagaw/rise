...
const IE_SCHOOL_DATA_TYPE_ID = process.env.IE_SCHOOL_DATA_TYPE_ID || "10"
...
async function listIeSchoolExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", IE_SCHOOL_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load IE School Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findIeSchoolExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listIeSchoolExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findIeSchoolExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find IE School Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The IE School Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "IE School")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findIeSchoolExcelRecordById(supabase, dataYearId, excelFileId)
      : await findIeSchoolExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No IE School Excel file found for the selected data year." },
        { status: 404 },
      )
    }
