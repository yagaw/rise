...
const IE_STUDENT_DATA_TYPE_ID = process.env.IE_STUDENT_DATA_TYPE_ID || "13"
...
async function listStudentExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", IE_STUDENT_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load Student IE Excel files: ${error.message}`)
  }
...
async function findStudentExcelRecordById(
  ...
  if (error) {
    throw new Error(`Failed to find Student IE Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The Student IE Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Student IE")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!record) {
      return NextResponse.json(
        { error: "No Student IE Excel file found for the selected data year." },
        { status: 404 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to load Student IE Excel file."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to save Student IE Excel file."
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
