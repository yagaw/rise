...
const ECCD_QLE_DATA_TYPE_ID = process.env.ECCD_QLE_DATA_TYPE_ID || "7"
...
async function listEccdQleExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", ECCD_QLE_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load ECCD QLE Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findEccdQleExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listEccdQleExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findEccdQleExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find ECCD QLE Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The ECCD QLE Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "ECCD QLE")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findEccdQleExcelRecordById(supabase, dataYearId, excelFileId)
      : await findEccdQleExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        {
          error:
            "No ECCD QLE Excel file found for the selected data year.",
        },
        { status: 404 },
      )
    }
