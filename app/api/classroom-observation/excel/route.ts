...
const CLASSROOM_OBSERVATION_DATA_TYPE_ID = process.env.CLASSROOM_OBSERVATION_DATA_TYPE_ID || "6"
...
async function listClassroomObservationExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", CLASSROOM_OBSERVATION_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load Classroom Observation Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findClassroomObservationExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listClassroomObservationExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findClassroomObservationExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find Classroom Observation Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The Classroom Observation Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Classroom Observation")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findClassroomObservationExcelRecordById(supabase, dataYearId, excelFileId)
      : await findClassroomObservationExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        {
          error:
            "No Classroom Observation Excel file found for the selected data year.",
        },
        { status: 404 },
      )
    }
