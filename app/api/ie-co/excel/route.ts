...
const IE_CO_DATA_TYPE_ID = process.env.IE_CO_DATA_TYPE_ID || "12"
...
async function listIeCoExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", IE_CO_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load IE CO Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findIeCoExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listIeCoExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findIeCoExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find IE CO Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The IE CO Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "IE CO")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findIeCoExcelRecordById(supabase, dataYearId, excelFileId)
      : await findIeCoExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No IE CO Excel file found for the selected data year." },
        { status: 404 },
      )
    }
