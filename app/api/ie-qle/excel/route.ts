...
const IE_QLE_DATA_TYPE_ID = process.env.IE_QLE_DATA_TYPE_ID || "11"
...
async function listIeQleExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", IE_QLE_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load IE QLE Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findIeQleExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listIeQleExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findIeQleExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find IE QLE Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The IE QLE Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "IE QLE")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findIeQleExcelRecordById(supabase, dataYearId, excelFileId)
      : await findIeQleExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No IE QLE Excel file found for the selected data year." },
        { status: 404 },
      )
    }
