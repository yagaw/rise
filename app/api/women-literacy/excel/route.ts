...
const WOMEN_LITERACY_DATA_TYPE_ID = process.env.WOMEN_LITERACY_DATA_TYPE_ID || "17"
...
async function listStudentExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", WOMEN_LITERACY_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load Women Literacy Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findStudentExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listStudentExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findStudentExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find Women Literacy Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The Women Literacy Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Women Literacy")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findStudentExcelRecordById(supabase, dataYearId, excelFileId)
      : await findStudentExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No Women Literacy Excel file found for the selected data year." },
        { status: 404 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to load Women Literacy Excel file."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to save Women Literacy Excel file."
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
