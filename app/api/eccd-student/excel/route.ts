...
const ECCD_STUDENT_DATA_TYPE_ID = process.env.ECCD_STUDENT_DATA_TYPE_ID || "14"
...
async function listStudentExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", ECCD_STUDENT_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load Student ECCD Excel files: ${error.message}`)
  }
...
async function findStudentExcelRecordById(
  ...
  if (error) {
    throw new Error(`Failed to find Student ECCD Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The Student ECCD Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Student ECCD")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!record) {
      return NextResponse.json(
        { error: "No Student ECCD Excel file found for the selected data year." },
        { status: 404 },
      )
    }
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to load Student ECCD Excel file."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to save Student ECCD Excel file."
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
