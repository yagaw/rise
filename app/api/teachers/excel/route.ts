...
async function findTeacherExcelRecord(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", TEACHER_DATA_TYPE_ID)
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle()

  if (error) {
    throw new Error(`Failed to find teacher Excel file: ${error.message}`)
  }

  return data as ExcelDataRecord | null
}

async function listTeacherExcelRecords(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const { data, error } = await supabase
  ...
  if (error) {
    throw new Error(`Failed to load teacher Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findTeacherExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The teacher Excel record does not have a file URL.")
  }
  ...
  if (response.ok) {
    return { buffer: await response.arrayBuffer(), storagePath }
  }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const records = await listTeacherExcelRecords(supabase, dataYearId)
    ...
    const record = excelFileId
      ? await findTeacherExcelRecordById(supabase, dataYearId, excelFileId)
      : await findTeacherExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No teacher Excel file found for the selected data year." },
        { status: 404 },
      )
    }
    ...
    workbookCache.set(cacheKey, parsedWorkbook)

    return NextResponse.json({
      excelFile: record,
      columns: parsedWorkbook.columns,
      rows: parsedWorkbook.rows,
    })
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Failed to load teacher Excel file."
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to save teacher Excel file."
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
