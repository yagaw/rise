...
const IE_TEACHER_DATA_TYPE_ID = process.env.IE_TEACHER_DATA_TYPE_ID || "9"
...
async function listIeTeacherExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", IE_TEACHER_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load IE Teacher Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findIeTeacherExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listIeTeacherExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findIeTeacherExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find IE Teacher Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The IE Teacher Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "IE Teacher")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findIeTeacherExcelRecordById(supabase, dataYearId, excelFileId)
      : await findIeTeacherExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        { error: "No IE Teacher Excel file found for the selected data year." },
        { status: 404 },
      )
    }
