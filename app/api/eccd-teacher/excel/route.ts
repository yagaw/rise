...
const ECCD_TEACHER_DATA_TYPE_ID = process.env.ECCD_TEACHER_DATA_TYPE_ID || "4"
...
async function listEccdTeacherExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", ECCD_TEACHER_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load ECCD Teacher Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findEccdTeacherExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listEccdTeacherExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findEccdTeacherExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find ECCD Teacher Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The ECCD Teacher Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "ECCD Teacher")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findEccdTeacherExcelRecordById(supabase, dataYearId, excelFileId)
      : await findEccdTeacherExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        {
          error:
            "No ECCD Teacher Excel file found for the selected data year.",
        },
        { status: 404 },
      )
    }
