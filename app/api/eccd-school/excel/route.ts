...
const ECCD_SCHOOL_DATA_TYPE_ID = process.env.ECCD_SCHOOL_DATA_TYPE_ID || "3"
...
async function listEccdSchoolExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", ECCD_SCHOOL_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load ECCD School Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findEccdSchoolExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listEccdSchoolExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findEccdSchoolExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find ECCD School Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The ECCD School Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "ECCD School")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findEccdSchoolExcelRecordById(supabase, dataYearId, excelFileId)
      : await findEccdSchoolExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        {
          error:
            "No ECCD School Excel file found for the selected data year.",
        },
        { status: 404 },
      )
    }
