...
const ECCD_CO_DATA_TYPE_ID = process.env.ECCD_CO_DATA_TYPE_ID || "8"
...
async function listEccdCoExcelRecords(
  ...
  const { data, error } = await supabase
    ...
    .eq("data_type", ECCD_CO_DATA_TYPE_ID)
    .order("created_at", { ascending: false })

  if (error) {
    throw new Error(`Failed to load ECCD CO Excel files: ${error.message}`)
  }

  return (data ?? []) as ExcelDataRecord[]
}

async function findEccdCoExcelRecord(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
) {
  const records = await listEccdCoExcelRecords(supabase, dataYearId)
  return records[0] ?? null
}

async function findEccdCoExcelRecordById(
  supabase: ReturnType<typeof createSupabaseAdminClient>,
  dataYearId: string,
  excelFileId: string,
) {
  ...
  if (error) {
    throw new Error(`Failed to find ECCD CO Excel file: ${error.message}`)
  }
...
async function downloadExcelFile(
  ...
  if (!record.url) {
    throw new Error("The ECCD CO Excel record does not have a file URL.")
  }
...
function buildWorkbook(columns: string[], rows: ExcelRow[]) {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "ECCD CO")
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const record = excelFileId
      ? await findEccdCoExcelRecordById(supabase, dataYearId, excelFileId)
      : await findEccdCoExcelRecord(supabase, dataYearId)

    if (!record) {
      return NextResponse.json(
        {
          error:
            "No ECCD CO Excel file found for the selected data year.",
        },
        { status: 404 },
      )
    }
